...
- The "`/CL` regex must never run over an object that already contains `/RC`" rule is enforced in code, not just by call order: `_strip_cl()` only rewrites the object when its regex matches exactly once (`subn == 1`); 2+ matches (e.g. a pre-existing `/RC` string that itself contains a spurious `/CL [` substring) is skipped and warned rather than risking corruption. `write_annotations()` also runs a pre-pass over every FreeText annot already present in the target PDF — pre-annotated targets from a prior tool (e.g. ChuYu/Acrobat) can carry the same malformed empty `/CL [ ]` + no `/IT` defect — and strips those too, because the final `/CL` integrity assertion sweeps *all* annots, not just pipeline-written ones, so pre-existing offenders must be cleaned or Phase 4 aborts even with `matches=[]`. Well-formed callouts (`/CL` + `/IT` both present) are exempted everywhere. See `docs/unknowns/arrow-connector-migration/implementation-notes.md`.
